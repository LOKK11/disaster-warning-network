...
centrality_scores["betweenness"] = nx.betweenness_centrality(G, weight="delay", normalized=True)

# Eigenvector centrality, use "weight" calculated in task 5 as weight
centrality_scores["eigenvector"] = nx.eigenvector_centrality(G, weight="weight", max_iter=500)

# Combine results into DataFrame

# Create DataFrame with degree centrality
centrality_df = pd.DataFrame.from_dict(centrality_scores["degree"], orient="index", columns=["degree"])
...
critical_nodes_df = centrality_df[centrality_df["top_n_count"] >= MIN_CENTRALITY_COUNT].sort_values("top_n_count", ascending=False)
...
nx.draw_networkx_nodes(G, pos=pos, node_color=node_colors, node_size=node_sizes, alpha=0.9)
